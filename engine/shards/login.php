...
*/
    ...
    $login_name = make_var_safe($_REQUEST['login_name']);
	...
	$verif_login = false;
	$result = mf_query("SELECT username, password, email, ID, userstatus, tentatives, next_tentative FROM users WHERE LOWER(username) = \"".mb_strtolower($login_name,'UTF-8')."\" LIMIT 1");
	if ($row = mysql_fetch_array($result))
		$verif_login = true;
	else if (stristr($login_name, "@")) {
		$result = mf_query("SELECT username, password, email, ID, userstatus, tentatives, next_tentative FROM users WHERE LOWER(email) = \"".mb_strtolower($login_name,'UTF-8')."\" LIMIT 1");
		if ($row = mysql_fetch_array($result))
			$verif_login = true;
	}
	if ($verif_login) {
		...
		if ($row['userstatus'] == "pending") {	
		...
		else if ($row['password'] == sha1($login_pass)) {
			$end_remember = 0;
			...
			else if (!strstr( $_SERVER['HTTP_REFERER'], "shard=login") && !strstr($_SERVER['HTTP_REFERER'], "shard=adduser"))
			...
			if ($row['tentatives'] == "2") {
				$dateStr = time() + 3600;
				$next_tentative = ", next_tentative = '$dateStr'";
			}
			mf_query("UPDATE users SET tentatives = tentatives + 1 $next_tentative WHERE ID=\"$row[ID]\" LIMIT 1");
    ...
    if (array_key_exists("logoutDidCheck", $_POST))
	{
		...
		//header("Location: index.php?shard=login&action=g_logout_success");
    break;
	...
	case "g_recover_pass":
		...
		if ((isset($_POST['nick_user'])) && (isset($_POST['email_user']))) {
			...
			$req = "SELECT ID, username FROM users where LOWER(username) =\"".mb_strtolower($nick_user,'UTF-8')."\" AND email=\"$email_user\" limit 1";
			$result = mf_query($req);
			...
			if ($row = mysql_fetch_array($result)) {
				...
				mf_query("UPDATE users SET reset_pass='".$auth."' WHERE username=\"$row[username]\" LIMIT 1");
	...
	if ($_REQUEST['auth']) {
		...
		if ($row = mysql_fetch_array($req)) {
			$thisContentObj->primaryContent = "<div style='font-size:1.3em;'>$LANG[MAIL_ACTIVATION11] <span class='bold'>$row[username]</span></div>
				<form method='post' action=\"".$_SERVER['PHP_SELF']."?shard=login&amp;action=g_procnewpass\" name='new_pass'>
				...
				<tr><td></td><td><input type='submit' value=\"$LANG[SUBMIT_EDIT]\" class='button'/></td></tr>
				</form></table></center></p>";
	...
	if ($_POST['auth']) {
		$error = "";
		$auth = make_var_safe($_REQUEST['auth']);
		if (array_key_exists("password" , $_REQUEST ) == TRUE )
			$password = make_var_safe(htmlspecialchars($_REQUEST["password"]));
		if (array_key_exists("vpassword" , $_REQUEST ) == TRUE )
			$vpassword = make_var_safe(htmlspecialchars($_REQUEST["vpassword"]));
		...
		if ($error)
			...
			$req = "SELECT reset_pass FROM users WHERE reset_pass=\"$_REQUEST[auth]\" LIMIT 1";
			$password = sha1($password);
			if ($result = mf_query($req)) {
				mf_query("UPDATE users SET password = \"$password\", reset_pass = '', tentatives = '0', next_tentative = '' WHERE reset_pass = \"$auth\" LIMIT 1");
				$thisContentObj->primaryContent = $LANG['PASS_RESET_SUCCESS'];
			}
			else
				$thisContentObj->primaryContent .= "<p>$LANG[PASS_RESET_ERROR]</p>";
		}
