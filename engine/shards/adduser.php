...
case "g_default":
	...
	if (isset($_POST['user'])) {
		...
		if (!$errormsg) {
			...
			mf_query("INSERT INTO users (username, password, birthdate, location, email, rating, datejoined, userstatus, avatar, lang) VALUES (\"$user\", '$sha1password', '$birthdate', '$location', '$email', 0, $datejoined, $verifyEmail, '$avatar_path', '$siteSettings[lang]' )");
			...
			if ($row = mysql_fetch_assoc($userID)) {
				if ($siteSettings['crypt_method'] == "blowfish")
					crypt_password($password,$userID);
	...
	<div style='display:table;border-spacing:0.8em;' class='fineC2'>
		...
		$thisContentObj->primaryContent .= "</div></form>";
...
case "add_user_fb":
	...
	mf_query("INSERT INTO users (username, password, email, rating, datejoined, userstatus, avatar, lang, facebookID) values (\"$user\", '$sha1password', '$fbemail', 0, $datejoined, $verifyEmail, '$avatar_path', '$siteSettings[lang]', '$fbuserID' )");
	...
	if (is_numeric($_REQUEST['i'])) {
		...
		if ($row=mysql_fetch_assoc($check)) {
			...
			if ($row['verifystring'] == $_REQUEST['v'])	{
				...
				$result = mf_query("SELECT password, username FROM users WHERE ID='$_REQUEST[i]' LIMIT 1");
				$result = mysql_fetch_assoc($result);
				setcookie("b6".$siteSettings['db']."username", "$result[username]", time()+864000000, "/");
				setcookie("b6".$siteSettings['db']."password", "$result[password]", time()+864000000, "/");
				setcookie("b6".$siteSettings['db']."userID", "$_REQUEST[i]", time()+864000000, "/");
				header("Location: ".make_link("login","&action=g_confirm_success"));
				break;
			}
			else
...
case "g_resendmail":
	...
	if ($row = mysql_fetch_assoc($userID)) {
		...
		if ($check=mysql_fetch_assoc($check))
...
case "g_remove":
	...
	if($_REQUEST['ID'] == $CURRENTUSERID) {
		if (isset($_POST['delete_conf'])) {
			...
			if ($row = mysql_fetch_assoc($user_posted))
