...
// admin.php
    ...
    if (isInGroup($CURRENTUSER, 'admin') || isInGroup($CURRENTUSER, 'sysadmin')) {
		...
		if (isInGroup($CURRENTUSER, 'sysadmin')) {
			...
			mf_query("UPDATE settings SET lang='$_POST[language]', loadavg='$loadavg', mod_rewrite='$mod_rewrite', siteurl=\"$url\", threadupdate='$threadupdate', postupdate='$postupdate', message=\"$message\", recaptcha_privKey = \"$recaptcha_privKey\", recaptcha_pubKey = \"$recaptcha_pubKey\" WHERE 1");
